...
class AdminCommon {
  ...
  async init() {
    try {
      ...
      // 안전망: 혹시라도 로딩이 남아있으면 제거
      try { this.showDashboard(); } catch (_) {}
      // 추가 안전망: 약간 지연 후 한 번 더 제거
      setTimeout(() => { try { this.showDashboard(); } catch(_) {} }, 300);
    } catch (error) {
  ...
  async checkAuth() {
    try {
      ...
      if (response.ok) {
        ...
        console.warn('[admin-common] /me 응답 실패 상태코드', response.status);
  ...
  // 대시보드 표시
  showDashboard() {
    // 전역 상태 플래그 추가 (CSS에서 강제 제어)
    try { document.body.classList.add('admin-ready'); } catch(_) {}
    // *** 이 함수가 호출되면 무조건 로딩 화면을 먼저 숨깁니다. ***
    this.hideLoading();
